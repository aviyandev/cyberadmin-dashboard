...
<html lang="{{ app()->getLocale() }}">
  <head>
    ...
    <title>Forgot Password | CyberAdmin</title>
  ...
  <body>
    <div class="cyber-card">
      ...
      <h2 style="font-family: var(--font-heading); font-size: 1.2rem; margin-bottom: 1.5rem;">FORGOT PASSWORD</h2>
      <p style="margin-bottom: 1.5rem; color: var(--text-muted);">Enter your email and we'll send you a password reset link.</p>
      
      @if (session('status'))
        <div style="margin-bottom: 1rem; padding: 1rem; background: rgba(0,255,0,0.1); border: 1px solid rgba(0,255,0,0.3); color: #0f0;">
          {{ session('status') }}
        </div>
      @endif
      ...
      <form method="POST" action="{{ route('password.email') }}">
        ...
        <button type="submit" class="cyber-btn">
          <i class="ph ph-paper-plane-right"></i> Send Reset Link
        </button>
      </form>
